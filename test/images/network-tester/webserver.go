...
func (s *State) serveStatus(w http.ResponseWriter, r *http.Request) {
	...
	go s.Logf("Declaring failure for %s with %d sent and %d received and %d peers", *service, len(s.Sent), len(s.Received), *peerCount)
	fmt.Fprintf(w, "fail")
}
...
func contactOthers(state *State) {
	...
	for start := time.Now(); time.Since(start) < timeout; time.Sleep(sleepTime) {
		...
		state.Logf("%v has %v endpoints (%v), which is less than %v as expected. Waiting for all endpoints to come up.", *service, len(eps), eps, *peerCount)
	}
